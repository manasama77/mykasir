...
WHERE DATE(tbl_list_penjualan_event.date_create) >= '$tanggal_awal' AND DATE(tbl_list_penjualan_event.date_create) <= '$tanggal_akhir'");

$q_sum_penjualan = mysqli_query($con, "SELECT SUM(lpe.total_harga) AS sum_grand_total FROM tbl_list_penjualan_event AS lpe WHERE DATE(lpe.date_create) >= '$tanggal_awal' AND DATE(lpe.date_create) <= '$tanggal_akhir'");
...
<head>
    ...
    <title>UD Mandiri Cahaya Abadi - Laporan Penjualan Discount <?=$tanggal_awal;?> s/d <?=$tanggal_akhir;?></title>
