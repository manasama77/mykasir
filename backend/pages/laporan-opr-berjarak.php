...
<head>
	...
	<title>UD Mandiri Cahaya Abadi - Laporan Beban Operasional</title>
		...
		<div class="col-lg-12">
			...
			<tbody>
				...
				<tr bgcolor="#FCFCFC">
					...
					<th style="text-align:left;">Rp. <?php 
						...
						// JUMLAH TOTAL POSTING 
					$q_jumlah_p =  mysqli_query($con, "SELECT SUM(nominal)FROM trx
						WHERE
						DATE(trx.tgl_reg) >= '$tgl_awal' AND DATE(trx.tgl_reg) <= '$tgl_akhir' AND status = 0
						");

					if (FALSE === $q_jumlah_p) die("Select sum failed: ".mysqli_error);
					$r_jumlah_p = mysqli_fetch_row($q_jumlah_p);
					$fa_jumlah_posting = $r_jumlah_p[0];
					$fa_jumlah_posting_format=number_format($fa_jumlah_posting,2,",",".");
			...
			<thead>
				<tr>
					...
					<th>
						<h4 class="header">POSTING LAPORAN BIAYA OPERASIONAL</h4>
						<h5 class="header">Jumlah Transaksi Belum Posting : <b><?=$fa_jumlah_posting_format;?></b></h5>
						<form name="operasional" method="post" action="all_posting_operasional.php">
							<input name="posting" type="submit" value="posting" onclick="myFunction()">
							<input type="hidden" name="nominal" value="<?=$fa_jumlah_posting;?>">
							<input type="hidden" name="leadger" value="10001">
							<input type="hidden" name="ket" value="POSTING DARI OPERASIONAL">
